...
if(args[0] === 'aç') {
    ...
    message.channel.send(':srowdoru: Başarılı Bir Şekilde SA-AS Sistemi `Açıldı!`')
      db.set(`saas_${message.guild.id}`, "acik")
  return
}
if (args[0] === 'kapat') {
  db.delete(`saas_${message.guild.id}`)
message.channel.send(':srowyanl: Başarılı Bir Şekilde SA-AS Sistemi `Kapatıldı!`')
      db.set(`saas_${message.guild.id}}`, "kapali")
return
}
  message.channel.send(':srowyanl: Lüten `aç` ya da `kapat` yazın!')
};
exports.conf = {
 enabled: true,
 guildOnly: false,
